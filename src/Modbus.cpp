 ...
 void Modbus::receivePDU(byte* frame) {
     ...
     switch (fcode) {
         ...
         case MB_FC_WRITE_REGS:
             ...
             this->writeMultipleRegisters(frame,field1, field2);
         break;
...
{
    // ---- VALIDATION ----
    if (numoutputs < 1 || numoutputs > 0x007B) {
        this->exceptionResponse(MB_FC_WRITE_REGS, MB_EX_ILLEGAL_VALUE);
        return;
    }

    const word expectedByteCount = numoutputs * 2;
    ...
    _len = 5;   // FC16 response PDU length
    _frame = (byte*)malloc(_len);
    ...
    _frame[1] = startreg >> 8;
    _frame[2] = startreg & 0xFF;
    _frame[3] = numoutputs >> 8;
    _frame[4] = numoutputs & 0xFF;
